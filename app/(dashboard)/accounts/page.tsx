              ...
              {(() => {
                ...
                return (
                  <div className="rounded-xl bg-warn/10 border border-warn/25 px-3 py-2.5 mb-2.5 text-[11.5px] leading-relaxed">
                    <div className="font-semibold text-warn mb-1">{diagData.sessionDead ? '⚠️ Действие оборвалось посреди цикла' : '⚠️ Прокси моргнул на одном действии'}</div>
                    <div className="text-ink/80 space-y-1.5">
                      <p>Сессия аккаунта <b>жива</b> (иначе «перечитать ник» не сработал бы) — это <b>не баг браузера</b>, менять Chromium бесполезно, и <b>не</b> значит, что сессия мёртвая.</p>
                      {diagData.sessionDead ? (
                        <p>Причина была в коде: при долгой навигации браузерный контекст закрывался фоновым таймером <b>посреди цикла</b> → следующие действия падали «login_required» на уже закрытом окне. <b>Исправлено (build-121)</b> — пока идёт цикл, контекст больше не закрывается.</p>
                      ) : (
                        <p>Прокси не ответил на ОДНОМ действии («моргнул») — разовый транзиент. Это действие повторится в следующем цикле, на остальные больше не влияет (сессия живёт дальше).</p>
                      )}
                      <p className="font-medium text-ink">Если повторяется ПОСЛЕ обновления (build-121+):</p>
                      <p>1) Проверь, что прокси в стране аккаунта и стабилен (лучше резидентный/мобильный).<br />2) Если «login_required» реально не уходит — сессия устарела: <b>войди заново по паролю</b> (кнопка «Войти заново»).</p>
                    </div>
                  </div>
                )
              })()}
