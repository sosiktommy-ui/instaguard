...
- [~] **Фаза 2 (🔴 ретраи — §4.2 + §4.5 + §4.6):** §4.2 СДЕЛАНО (ретраи 2captcha на UNSOLVABLE/NO_SLOT/таймаут
  внутри `solveRecaptchaV2`/`solveHCaptcha`/`solveFunCaptcha`, фатальные коды — сразу проброс). ⬜ §4.5
  (`captchaTried` bool → `captchaAttempts` счётчик в `attemptLogin` — сейчас 1 попытка на входе; но ретраи
  есть на уровне 2captcha, поэтому не блокер) + §4.6 (дедлайны) — ОСТАЛОСЬ.
- [ ] **Фаза 3 (🟠 enterprise-параметры — §4.3 + §4.7):** надёжный enterprise-детект в DOM-ветке,
  эксперимент pageurl (a/b) + `data-s`, расширенная трасса. Снизить долю UNSOLVABLE.
- [ ] **Фаза 4 (🟠 остальные пути — §5–§9):** капча в `loginByState`; проверка 2FA/challenge-экранов
  с капчей; кнопка Continue во фреймах; `LOGIN_TIMEOUT_MS` под новый бюджет.
