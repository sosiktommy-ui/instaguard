...
// Порядок действий на цель (запрос пользователя): ПОДПИСКА ВСЕГДА ПЕРВОЙ (главное правило),
// ДИРЕКТ последним (после «прогрева» цели). Средние (лайк/коммент/сторис) — в СЛУЧАЙНОМ порядке
// каждый раз (анти-робот: бот не должен делать один и тот же сценарий одинаково 1000 раз).
// Ранг средних считается ОДИН раз на задачу (стабилен внутри сортировки).
function orderTasks(a) {
  return [...a]
    .map((t) => ({ t, r: t.type === 'follow' ? 0 : t.type === 'dm' ? 100 : 10 + Math.random() * 80 }))
    .sort((x, y) => x.r - y.r)
    .map((x) => x.t)
}
