...
// 2026-07-20: поднято 2→8. Живой кейс: сессия в БД ПОДТВЕРЖДЕНА живой (🔤 «перечитать ник» работает
// в свежем окне с той же кукой), но ротирующий прокси (IP на каждом соединении) заставляет IG
// сбрасывать локальную куку почти на каждом действии → лимит 2 исчерпывался после ~2 действий, и
// весь остаток цикла (follow+like+story+dm по нескольким целям) валился login_required. Эталон
// server-side ВАЛИДЕН, поэтому вливать его обратно чаще — не «долбёж мёртвой сессии», а нормальное
// восстановление живой: 8 хватает на полный цикл. Всё ещё ограничено (не бесконечно) — если кука
// падает 8 раз, сессия/прокси реально нестабильны → CHALLENGE. ⚠️ Полное лекарство — sticky-прокси
// (1 аккаунт = 1 IP): никакой код не победит per-connection ротацию IP до конца, лишь смягчает.
const MAX_SESSION_HEALS = 8
