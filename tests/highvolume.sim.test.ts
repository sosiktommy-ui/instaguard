...
describe('высокие обороты: 120 подписок — никто не теряется, лимит не превышен', () => {
  ...
  it('НАСТРАИВАЕМЫЕ лимиты: подняли follow 15→60 → все 120 за ~2 дня (быстрее, но выше риск)', () => {
    // Лимиты пользовательские (Настройки → Дневные лимиты). Выше cap = быстрее оборот. Дрип 12/цикл
    // × 8 = 96 попыток/день, поэтому при cap 60 узкое место — снова cap (60/день) → 120/60 = 2 дня.
    const hi: Caps = { ...DAILY_CAPS, follow: 60 }
    const r = simulate(120, 12, hi)
    expect(r.doneSize).toBe(120)
    expect(r.maxPerDay).toBeLessThanOrEqual(60)
    expect(r.days).toBe(2)                          // против 8 при дефолте 15 — прямая зависимость от cap
  })
