...
#### fix(UI): справка «Что где находится» сворачивается по умолчанию + ширина колонок формы создания

- **`settings/page.tsx`:** блок «Что где находится» был развёрнут по умолчанию (6 строк сразу). Добавлен `showHelp` (стартует `false`) + заголовок-переключатель со стрелкой (`ChevronDown`, поворачивается при раскрытии).
- **`triggers/page.tsx:771`:** колонки формы «Создать кампанию» (Аккаунты · Событие · Настройка) были равными (`grid-cols-3`) — колонка «Настройка» была самой длинной по контенту и разворачивала форму слишком высоко вниз, из-за чего снизу переставало быть заметно, что там ещё что-то есть. Ширины сделаны неравными: `lg:grid-cols-[0.8fr_0.8fr_1.4fr]` (без аккаунтов — `[0.8fr_1.2fr]`) — «Настройка» шире и идёт больше вширь, «Аккаунты»/«Событие» уже.
